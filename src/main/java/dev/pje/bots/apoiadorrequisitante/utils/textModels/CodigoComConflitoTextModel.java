...
@Component
public class CodigoComConflitoTextModel implements AbstractTextModel {
	 ...
	 * [MR#xx/link para o merge] foi fechado, pois o código apresenta conflito ou outro problema que impede que seja juntado ao branch de destino.<br/> 
	 * O desenvolvedor [~desenvolvedor.xyz] deve revisar seu código, adaptar a solução e ressubmete-la.
	 */
	public String convert(MarkdownInterface markdown) {
		...
		markdownText
			...
			.append(markdown.normal(", pois o código apresenta conflito ou outro problema que impede que seja juntado ao branch de destino."))
			.append(markdown.newLine())
			.append(markdown.normal("O desenvolvedor "))
			.append(markdown.normal(referUser))
			.append(markdown.normal(" deve revisar seu código, adaptar a solução e ressubmete-la."));
